...
> 注：原文此处配有一张 Xcode 工程目录截图，展示 AppName / AppNameTests 两个顶级分组的关系。该图片在迁移前的备份中已经丢失，所以这里只保留文字描述：
>
> 一个新建的 iOS app 在 Xcode 中包含两个顶级组（`AppName`、`AppNameTests`），后者包含 example XCTestCase 类与一组功能 / 性能测试示例。
